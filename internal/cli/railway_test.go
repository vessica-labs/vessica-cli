...
func TestLinkRailwayWorkDirUsesExistingProjectAndEnvironment(t *testing.T) {
	...
	t.Setenv("VES_TEST_COMMAND_LOG", logPath)
	t.Setenv("RAILWAY_TOKEN", "test-token")
	cfg := config.Config{Hosted: config.HostedConfig{WorkspaceID: "workspace-1", WorkspaceName: "Workspace One", ProjectID: "project-1", EnvironmentID: "environment-1"}}
	...
	want := resolvedWorkDir + "|link --project project-1 --environment environment-1 --workspace Workspace One --json\n"
...
func TestRunRailwayFallsBackToCLISessionWhenOAuthCannotSeeWorkspace(t *testing.T) {
	home := t.TempDir()
	bin := filepath.Join(home, "bin")
	if err := os.MkdirAll(bin, 0o755); err != nil {
		t.Fatal(err)
	...
	script := filepath.Join(bin, "railway")
	content := "#!/bin/sh\nif [ -n \"$RAILWAY_API_TOKEN\" ]; then echo 'Workspace My Projects not found.' >&2; exit 1; fi\nprintf 'session:%s' \"$*\"\n"
	if err := os.WriteFile(script, []byte(content), 0o755); err != nil {
		t.Fatal(err)
	}
	t.Setenv("HOME", home)
	...
	t.Setenv("RAILWAY_TOKEN", "")
	t.Setenv("RAILWAY_API_TOKEN", "")
	if err := auth.SaveOAuth(&auth.OAuthCredential{Provider: "railway", ClientID: "client", TokenURL: "https://example.invalid", AccessToken: "other-account", ExpiresAt: time.Now().Add(time.Hour), UpdatedAt: time.Now()}); err != nil {
		t.Fatal(err)
	}
	output, err := runRailway(context.Background(), "", nil, "link", "--workspace", "My Projects", "--json")
	if err != nil || string(output) != "session:link --workspace My Projects --json" {
		t.Fatalf("output=%q err=%v", output, err)
	}
}

func TestRunRailwayReplaysStdinWhenFallingBackToCLISession(t *testing.T) {
	home := t.TempDir()
	bin := filepath.Join(home, "bin")
	if err := os.MkdirAll(bin, 0o755); err != nil {
		t.Fatal(err)
	...
	output, err := runRailway(context.Background(), "", strings.NewReader("expected-value\n"), "variable", "set", "KEY", "--stdin")
	if err != nil || string(output) != "expected-value" {
		t.Fatalf("output=%q err=%v", output, err)
	}
}
