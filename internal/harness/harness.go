...
func Detect(root string) Detected {
	...
	case fileExists(filepath.Join(root, "pyproject.toml")) || fileExists(filepath.Join(root, "requirements.txt")):
		d.Stack = "python"
		d.PreviewCommand = "python -m http.server 8000"
		d.PreviewPort = 8000
		d.Healthcheck = "http://localhost:8000/"
		d.BuildCommand = "python -m compileall ."
		d.TestCommand = "pytest"
		d.LintCommand = "ruff check ."
	default:
		d.PreviewCommand = "echo 'configure preview.command in .vessica/harness.yaml'"
		d.BuildCommand = "echo 'configure build.command'"
		d.TestCommand = "echo 'configure test.command'"
		d.LintCommand = "echo 'configure lint.command'"
	}
	return d
}
...
func defaultDoc(name string, d Detected) string {
	...
	return fmt.Sprintf("# %s\n\nGenerated by Vessica harness sync.\n\nDetected stack: %s\n\n## Notes\n\nUpdate this document to encode durable project constraints for agents.\n", title, d.Stack)
}
