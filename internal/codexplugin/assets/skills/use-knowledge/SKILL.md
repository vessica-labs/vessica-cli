...
- Durable understanding: `ves memory retrieve "<query>" --limit 8 --rerank auto --json`, `ves memory get <id> --json`, and confirmed `memory add|update|supersede|archive` mutations. Keep `ves memory search` for administrative lexical inspection. Create an `instruction` only when the user explicitly requests durable guidance.
