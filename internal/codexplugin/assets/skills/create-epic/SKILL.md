...
Persistence requires confirmation. After confirmation, run `ves epic add --spec-file <path> --yes --idempotency-key epic-<unique> --json`. In a hosted workspace this creates the canonical repository-scoped Vessica record. If Linear is connected, Vessica also projects the epic and later ticket graph to the configured Linear project; never call Linear directly. Starting the design/build run is a separate confirmation boundary handled by `dispatch-epic`.
...
On `invalid_spec`, repair missing titles, unknown dependency keys, duplicates, or cycles and draft again. On `hosted_create_failed` or legacy `hosted_publish_failed`, retry the same `ves epic add` command with the same idempotency key; do not invent a separate publish command or create a second epic. A Linear projection failure does not erase the canonical Vessica record; report it and retry through Vessica instead of recreating the epic or calling Linear directly. On `confirmation_required`, stop and request confirmation.
