...
ecosystems and nested workspace roots, respects each lockfile's package manager,
pre-installs every dependency plan, and preserves package caches. Later runs
restore that disk, fetch only the Git delta, and skip dependency installation
when lockfiles and harness commands are unchanged. Refreshed checkpoints are
promoted only after a successful full run. Secrets, Railway variables, and
network mode are injected per run and are never captured in the checkpoint.
