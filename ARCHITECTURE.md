...
Repository checkpoints are the warm source of toolchains, the checkout, and dependencies. Their reviewable specification records the detected stack, package manager, manifests, workspace roots, and required tools. Integration worktrees link the immutable baked dependency tree directly; reflink, offline reconstruction, and ordinary installation remain stale-checkpoint fallbacks. After a successful delta sync, a background job forks and scrubs the sandbox, verifies the checkpoint root is clean, captures the new generation, and atomically advances repository metadata. Each result is emitted as an infrastructure stage.
