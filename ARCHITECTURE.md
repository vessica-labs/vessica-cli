...
Repository checkpoints are the warm source of toolchains, the checkout, and dependencies. Their reviewable specification records every detected stack, authoritative package manager, nested manifest, workspace root, required tool, and install recipe; mixed repositories warm every ecosystem rather than selecting the first manifest. The environment fingerprint includes dependency manifests, lockfiles, and the harness validation contract, so source-only deltas stay warm while environment changes invalidate deterministically. Integration worktrees project baked dependency trees into local directories using copy-on-write reflinks, with offline reconstruction and ordinary installation as stale-checkpoint fallbacks; cross-worktree dependency symlinks are forbidden. A refreshed snapshot remains a candidate until the producing run completes all gates. Only then does a background job fork and scrub the sandbox, verify the checkpoint root is clean, capture the new generation, and atomically promote repository metadata. Each result is emitted as an infrastructure stage.

Toolchain checkpoints carry a fingerprinted runtime attestation, an engine-managed MCP inventory, and may carry the exact checksum-identified worker binary. Warm forks verify these local contracts first and retain full verification and verified download fallbacks. Explicit localized XS epics use deterministic planning artifacts. Build orchestration overlaps lint and architecture checks with the build lane, preserves build-before-test ordering, and serially reruns all gates after any repair.

The planning call may return a validated single ticket for `xs` work. The durable ticketization phase reuses that result instead of making a second model call; larger work retains dependency-aware ticketization. Split tickets declare owned paths, and independently runnable tickets cannot own the same path; shared-file integration must be represented as a dependent ticket. Coding agents receive a bounded, version-matched context packet containing current-run planning artifacts, relevant CLI/receipt contracts, and focused validation guidance. The engine—not the coding agent—owns repository-wide build, lint, test, preview, and receipt gates.
